...
It features a broad coverage through unit tests and extensive \gls{api} documentation as well as didactic examples (see \cref{ch:ff:examples}).
...
\begin{enumerate}
    ...
    \item \gls{ggm} bases of arbitrary dimension $d$ from \cref{eq:ff:basis:ggm}.
\end{enumerate}
...
\end{lstlisting}
where \verb|F| is the dephasing filter function $\FF_{zz}(\omega)$ as we only defined a single noise operator.
