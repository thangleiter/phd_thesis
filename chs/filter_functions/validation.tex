...
\end{equation} \todo{find factor $2\pi$}
...
It's a sinusoid with amplitude $A_i \sim \text{Rice}(0, \sigma_i^2)$\marginnote{
    $\text{Rice}(\nu, \sigma)$ is the Rice distribution with probability density function
    \begin{equation*}
        \rho(x) = \frac{x}{\sigma^2}\exp{-\frac{x^2 + \nu^2}{2\sigma^2}}I_0\left(\frac{x\nu}{\sigma^2}\right)
    \end{equation*}
    with $I_0(z)$ the modified Bessel function of the first kind with order zero \cite{RiceDistributionWiki}.
    It describes the probability distribution of the distance from the origin of a point drawn from a bivariate normal distribution with mean $\nu$ and standard deviation $\sigma$.
}, frequency $\omega_i$, and phase $\phi \sim U(0, 2\pi)$\marginnote{
