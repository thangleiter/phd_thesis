...
\begin{split}
    \decayamps_{\alpha\beta,kl} = \sum_{g,g^\prime=1}^{G}\int_{-\infty}^\infty\ddf{\omega} &
    ...
    \eqqcolon\sum_{g,g^\prime=1}^{G}\int_{-\infty}^\infty\ddf{\omega} &
    S_{\alpha\beta}(\omega) \FF_{\alpha\beta,kl}\gth{gg^\prime}(\omega)
\end{split}
\end{equation}
...
\begin{equation}
    \int_0^t\dd{t'} \rightarrow \sum_{g^\prime=1}^{g-1}\int_{t_{g^\prime-1}}^{t_{g^\prime}}\dd{t'} + \int_{t_{g-1}}^{t}\dd{t'}.
\end{equation}
...
\left\lbrace\sum_{g^\prime=1}^{g-1}\int_{t_{g^\prime-1}}^{t_{g^\prime}}\dd{t'} + \int_{t_{g-1}}^{t}\dd{t'}\right\rbrace
...
\begin{figure}[tbp]
    \centering
    \includegraphics{img/pdf/filter_functions/benchmark_MC_vs_FF}
