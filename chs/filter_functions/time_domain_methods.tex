...
\begin{equation}\label{eq:ff:control_hamiltonian:qft}
    H_\mr{c}(t) = \sum_{\langle i,j\rangle} I_i(t)\px^{(i)} + Q_i(t)\py^{(i)} + J_{ij}(t)\pz^{(i)}\otimes\pz^{(j)}
\end{equation}
...
H_\mr{n}(t) = \sum_{\langle i,j\rangle} b_{I}(t)\px^{(i)} + b_{Q}(t)\py^{(i)} + b_{J}(t)\pz^{(i)}\otimes\pz^{(j)}
\end{equation}
