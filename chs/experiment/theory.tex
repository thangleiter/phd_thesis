...
What is more, these issues are exacerbated when the structures need to be thinned down to a membrane~\cite{Descamps2021,Kindel2026}.
Therefore, to accommodate \acrfullpl{gdqd} next to the \acrfull{oaqd} in the membrane, the devices measured in \thispart are fabricated on doped heterostructures hosting a \gls{2deg}.
In the following, I discuss the optical behavior of these structures under illumination to develop an intuition for the measurements presented later on.\sidenote{
    The \emph{electrical} behavior is another matter; there, illumination can lead to the creation, trapping, and de-trapping of free charge carriers that alter the transport properties of the device, leading to electrical hysteresis and instability~\cite{Fujita2021,Shetty2022,Wang2023,Reznikov2024,Kindel2026}.
}
...
Offset by the same absolute amount in the opposite direction is the valence band, which is fourfold degenerate close to $\Gamma$ with heavy and light holes with effective masses $m^{\ast}_{\mr{hh}}/m_{\mr{e}} = 0.34$ and $m^{\ast}_{\mr{lh}}/m_{\mr{e}} = 0.09$, respectively~\cite{Miller1984a}.
...
Introducing confinement in one direction, for example by doping a \GaAsAlGaAs heterojunction or growing a \ch{GaAs} \acrfull{qw} sandwiched between two layers of \ch{AlGaAs}, lifts the fourfold degeneracy of light and heavy holes and leaves -- under typical conditions -- the latter as the valence band ground state.
...
\begin{equation}\label{eq:exp:theory:qcse:bohr_radius_2d}
    a_{\mr{B}}\gth{2D} = \frac{2\pi\eps_0\eps_{\mr{r}}\hbar^2}{\mu e^2} = \qty{6}{\nano\meter}
\end{equation}
...
This is the \acrfull{qcse}~\cite{Miller1984}.
...
\end{equation}
The ground state energy is then \qty{14}{\milli\electronvolt} (\qty{3}{\milli\electronvolt}) above (below) the conduction (valence) band edge, corresponding to \qty{6}{\percent} (\qty{2}{\percent}) of the respective band offsets and implying that the infinite-well approximation is acceptable,\sidenote[][*-1]{
...
For the latter, \textcite{Miller1985} found that finite-well properties could be reproduced by using effective well widths with infinite-well models.
...
Because the exciton Bohr radius in the plane, $a_{\mr{B}}\gth{2D}$ (\cref{eq:exp:theory:qcse:bohr_radius_2d}), is much smaller than both the oscillator length $\xi$, we separate the exciton's center-of-mass motion from the relative electron-hole motion in the plane~\cite{Kavokin1994}.
Since furthermore the \gls{qw} well width $L$ is not much larger than the bulk Bohr radius $a_{\mr{B}}\gth{3D}=2a_{\mr{B}}\gth{2D}$, we initially also separate electron and hole motions normal to the plane from the in-plane motion, allowing us to make the product ansatz
...
\end{equation}
with\sidenote{
    Note that \textcite{Karimi2014} miss a factor $\flatfrac{1}{\sqrt{2\pi}}$ in the normalization.
}~\cite{Karimi2014}
...
\begin{align}
    ...
    F(0) &= \frac{1}{a_{\mr{B}}\gth{2D}}\sqrt{\frac{2}{\pi}}, \label{eq:exp:theory:osc:relative}
\end{align}
\end{subequations}
and where $m$ is the free electron mass.
\Cref{eq:exp:theory:osc:relative} holds exactly only for strictly two-dimensional excitons~\cite{Andreani1990a} and $\ket*{u_{\mr{c}(\mr{hh})}}$ are the lattice-periodic parts of the Bloch functions of the conduction and valence bands, respectively.
...
This in turn implies from \cref{eq:exp:theory:qcse:trap:ho:eps} that the optically resolvable exciton level spacing in a radially symmetric trap is given by $\Delta E = 2\hbar\omega=\qty{1}{\milli\electronvolt}$, a factor of two larger than assumed by \textcite{Descamps2021}.
...
The top panel shows the exciton transition energy $\Delta E_{np0}$ for the optically active $\ell = 0$ states in solid lines.\sidenote[][*-2]{
    Note again that we neglected the exciton binding energy, which decreases with increasing field and hence slightly reduces the red shift~\cite{Miller1984}.
}
