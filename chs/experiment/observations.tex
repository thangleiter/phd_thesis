...
For details about the fabrication process, refer to \citerr{Descamps2021}{Kindel2026}.
...
Several old samples fabricated during the work of \textcite{Descamps2021} appeared to have aged, resulting in broken gates or otherwise unconventional behavior.
...
This effect has already been observed by \textcite{Pinczuk1984}.
...
\textcite{Bockelmann1990} showed that in low-dimensional electron gases electron cooling by phonon emission is limited by a finite scattering rate.
Thus, under continuous excitation a quasi-equilibrium is established as hot electrons cannot loose heat quickly enough.
Although their simulations were performed for higher temperatures and smaller confinement widths, their results of electron temperatures in the range of \qty{2}{\kelvin} for lattice temperatures of \qty{300}{\milli\kelvin} are comparable to ours.
...
Compared to the results of \textcite{Kamburov2017}, the \gls{pl} spectra obtained here are much flatter over energy, with the \gls{pl} peak typically close to the middle between gap and Fermi edge.
Conversely, \textcite{Kamburov2017} observed a strong peak at the band gap,\sidenote{
    \Cf also \textcite{Gabbay2008}, who observe all but no \gls{fes} in samples nominally comparable to ours.
}
...
\textcite{Descamps2021} hypothesized that the removal of the \ch{GaAs} substrate and the associated change in strain leads to this lowering of the band gap.
However, this effect was also already observed by \textcite{Pinczuk1984} in \enquote{bulk} modulation-doped \ch{GaAs} \glspl{qw}.
...
\begin{figure}
    ...
    \caption[
        ...
        The behavior is qualitatively similar but the overall end-to-end efficiency lower by an order of magnitude for gates on the bottom (as-grown buried) side.
...
This is puzzling at first, as in the latter case there is an additional semi-transparent gate\sidenote{
...
Perhaps surprisingly, the gates on the \emph{bottom} side of the membrane display this behavior, \ie, the gates on the as-grown surface.\sidenote{
    I note that the sample of \cref{fig:exp:pl:honey_H13_stark_shift_vs_gate} was fabricated on the same heterostructure as the device in Figure 4(d) of \citer{Descamps2023}, which showed little to no electrical hysteresis unlike most other samples investigated by \textcite{Descamps2021}.
}
...
As can be seen from the cut at \qty{-2.3}{\volt} (orange), the line shape appears to consist of three individual emission lines assigned by \textcite{Descamps2021} as the neutral, singly, and doubly negatively charged excitons. % TODO: exciton/trion mixing ref
...
The former describes inhomogeneous broadening due to noise faster than the data acquisition time, whereas the latter is the homogeneous broadening due to for example the finite lifetime of the emitting state\sidenote[][*-3]{
...
Whereas for $\VT = \qty{-2.08}{\volt}$ the observed \gls{pl} features are similar to those in \cref{fig:exp:pl:doped_M1_05_49-2_power},
...
The increase in effective electric field towards the edge of an exciton trap is reminiscent of the simulation of the same type of experiment for an undoped \gls{qw} by \textcite[Figure~6.4]{Descamps2021}, although it was not explained there.
...
For well-resolved \gls{pl} lines, a common practice is to spectrally filter the \gls{pl} radiation while sweeping the excitation laser wavelength, thus obtaining the luminescence flux as function of excitation energy, $\Phi_{\mr{PLE}}(E_{\mr{exc}}; E_{\mr{det}})$, for a single detection energy $E_{\mr{det}}$.
By contrast, a \gls{pl} measurement is performed at a fixed excitation energy and yields $\Phi_{\mr{PL}}(E_{\mr{det}}; E_{\mr{exc}})$.
...
\begin{equation}\label{eq:exp:ple}
    I(E_{\mr{exc}}) = \int_{0}^{E_{\mr{exc}}}\dd{E_{\mr{det}}} \Phi(E_{\mr{det}}, E_{\mr{exc}}),
\end{equation}
...
In fact, their \gls{pl} emission is well fitted with the lineshape proposed by \textcite{Esser2001}, who describe the trion peak as a convolution of the exciton peak, given by a hyperbolic secant, with a thermally broadened exponential edge.
...
Interpolating the harmonic potential found by \textcite{Descamps2021} for a smaller trap to this trap geometry,\sidenote{
...
Each layer $i$ may consist of a different dielectric material characterized by a (complex) relative permittivity $\epsilon_{r,i}$.\sidenote[][*-1]{
...
Finally, note that for a thorough accounting of in- and out-going field amplitudes, excitonic effects should be included, for example using the approach by \textcite{DAndrea1990}.
...
In the first case, the translational invariance along $x$ leading to a plane-wave spatial dependence\sidenote{
    Recall that $\gamma_i = \sqrt{\eps_{r,i}k_0^2 - k_x^2}$.
}
that was assumed previously is replaced by a superposition of plane waves weighted with a normally distributed amplitude,\sidenote{
...
In the second case, \textcite{Langevin2024} consider an AC current $I$ flowing through a translationally invariant, one-dimensional wire along $y$ at $x=x_{\mr{s}}$.
...
The discrepancy between the two results indicates that the thicker barriers suggested here result in worse coupling to the Gaussian target mode optimized for by \textcite{Wu2020}.
