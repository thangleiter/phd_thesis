...
The package approaches \acrlong{daq} from the inverse direction: rather than inferring the spectrum properties from the time series data, users specify the properties they would like the resulting spectrum to have and the package chooses the correct parameters for \acrlong{daq} accordingly.

\section{Package design and implementation}\label{sec:speck:software:design}
\begin{marginfigure}[*-11]
    \input{img/tikz/spectrometer/speck_tree}
    \caption[\imgsource{img/tikz/spectrometer/speck_tree.tex}]{
        Source tree structure of the \pyspeck package.
        Driver wrappers are placed in the \codev{daq} subpackage.
        \codev{core.py} exports the \codev{Spectrometer} class.
    }
...
It is instantiated with an instance of a child class of the \code{DAQ} base class that implements an interface to various \gls{daq} hardware devices.\sidenote[][*9]{
...
\begin{margintable}
    ...
    \caption{
        Variable names used in \cref{ch:speck:theory} and their corresponding parameter names as used in \pyspeck and \codev{scipy.signal.welch()}~\cite{WelchScipy}.
    }
...
\begin{listing}[htpb]
    ...
    \end{pycode}
    \caption{
        \codev{DAQSettings} example showcasing automatic parameter resolution.
        \codev{n\_avg} determines the number of outer averages, \ie, the number of data buffers acquired and processed individually.
    }
...
\begin{marginlisting}[*-8]
    ...
    \end{pycode}
    \caption{
        Resolved settings for the same input parameters as in \cref{lst:speck:daq:settings} but for the \codev{ZurichInstrumentsMFLIScope} backend with hardware constraints on \codev{n\_pts} and \codev{fs}.
    }
    \label{lst:speck:daq:settings:mfli_scope}
\end{marginlisting}

If the instrument she'd chosen for data acquisition had been a Zurich Instruments MFLI's Scope module,\sidenote[][*6]{
    \url{https://docs.zhinst.com/labone_api_user_manual/modules/scope/index.html}
}
the same requested settings would have resolved to those shown in \cref{lst:speck:daq:settings:mfli_scope}.\sidenote[][*8]{
...
\begin{listing}[htpb]
    ...
    \caption{
        ...
        A \codev{MyDAQ} object (representing the instrument \codev{My}) is instantiated with a driver object (for instance a \href{https://github.com/microsoft/qcodes}{\qcodes} \codev{Instrument}).
        The instrument is configured with the given \codev{user\_settings}.
        Calling the generator function \codev{daq.acquire()} with the actual device settings returns a generator, iterating over which yields one data buffer per iteration.
...
The method returns the actual device settings which are then forwarded to the \code{acquire()} generator function.
...
Leveraging the \code{qutil.domains} module~\cite{Hangleiter_qutil}, more complex constraints such as sample rates restricted to an internal clock rate divided by a power of two\sidenote{
    See for example the implementation of the \href{https://git.rwth-aachen.de/qutech/python-spectrometer/-/blob/v2025.9.1/src/python_spectrometer/daq/atsaverage.py}{AlazarTech ATS9440} digitizer card.
}
can be specified.
...
\begin{listing}[htpb]
    ...
    \end{pycode}
    \caption{
        Exemplary code for a \codev{DAQ} implementation of some instrument with given driver class \codev{DeviceHandle} in the package \codev{mydriver}.
        The \code{MyDAQ} class is instantiated with a \codev{DeviceHandle} instance.
        Optionally, the \codev{DAQSettings} property can be overridden to implement hardware constraints or default values for data acquisition parameters.
        For this, the \codev{qutil.domains} module provides several classes that represent bounded domains and sets.
        The \codev{setup()} method parses the given acquisition settings and configures the instrument through the external driver interface \codev{handle.configure()}.
        The \codev{acquire()} method arms the instrument (if necessary) and loops over the number of outer averages, \codev{n\_avg}.
        ...
        Once acquisition is done, the method can return arbitrary metadata to the \codev{Spectrometer} object to attach to the stored data.
    }
...
\begin{marginlisting}
    ...
    \end{pycode}
    \caption{
        A simple \codev{procfn}, which converts amplified data back to the level before amplification.
        Note the token \codev{**\_} variable keyword argument that ensures no errors arise from other parameters being passed to the function.
        More complex processing chains can concisely be defined with \codev{qutil.functools.FunctionChain} that pipes the output of one function into the input of the next.
    }
...
\begin{marginlisting}
    \begin{pycode}[fontsize=\footnotesize]
        def derivative(xf, f, n=0, **_):
            return xf / (2j * pi * f)**n
    \end{pycode}
    \caption{A simple \codev{fourier\_procfn}, which calculates the \mbox{(anti-)}derivative.}
...
\begin{listing}[htpb]
    ...
    \caption{
        ...
        \codev{session} and \codev{device} are \gls{api} objects of the \codev{zhinst.toolkit} driver package.
        It is therefore possible to simply use the driver objects that are already in use in the measurement setup.
        The \codev{procfn} and \codev{processed\_unit} arguments help converting raw data into a more human-friendly unit.
    }
...
This is because the \gls{lia} low-pass filters the signal to suppress aliasing, and hence $n$ is the filter order.\sidenote[][*-4]{
    ...
    Some \gls{daq} devices perform internal aliasing rejection while others do not, and it is up to the user to ensure sufficient anti-aliasing.
}
...
\begin{listing}[htpb]
    ...
    \caption{
        ...
        Arbitrary key-value pairs can be passed to the \codev{take()} method, which are stored as metadata if they do not apply to any functions downstream in the data processing chain.
    }
...
\begin{figure}
    ...
    \caption[\imgsource{img/py/spectrometer/pyspeck_workflow.py}]{
        The \pyspeck plot shown in \cref{fig:speck:software:workflow:spectra} when setting \codev{spect.plot\_timetrace = True}.
...
Bob now observes that the noise spectra he has recorded display many sharp peaks in particular at high frequencies while the \oneoverf noise floor seems pretty consistent across different measurements.\sidenote{
    I once again refer readers to the lecture materials for details on the physical origins and mitigation strategies for different types of spectral signatures~\cite{Bluhm2021,Bluhm2022}.
}
...
In the case that \code{spect.plot_amplitude == True}, this new subplot shows the cumulative \gls{rms} in the band $[\fmin, f]$,
...
In his case, Bob notices that the \qty{400}{\hertz}-peak in the \code{"connected"} state dominates that spectrum, while higher harmonics are less harmful.
On the other hand, the \code{"lifted cable"} state somehow removed the peak at \qty{400}{\hertz} and instead introduced one at around \qty{900}{\hertz}!

\begin{figure}
    \centering
    \includegraphics{img/pdf/spectrometer/workflow_cumulative}
    \caption[\imgsource{img/py/spectrometer/pyspeck_workflow.py}]{
        The \pyspeck plot shown in \cref{fig:speck:software:workflow:spectra} when setting \codev{spect.plot\_cumulative = True}.
...
Thus, he rescales the spectra in terms of their relative powers expressed in \unit{\decibel}\sidenote{
...
In his case, Bob can tell that the \oneoverf noise is close to \qty{20}{\decibel} above the \gls{daq} noise floor at \qty{10}{\hertz}, and that the \qty{400}{\hertz} peak of the \code{"connected"} state is more than \qty{25}{\decibel} above the other spectra.
...
\begin{figure}
    ...
    \caption[\imgsource{img/py/spectrometer/pyspeck_workflow.py}]{
        ...
        Starting from the state in \cref{fig:speck:software:workflow:spectra}, we set \codev{spect.plot\_dB\_scale = True} as well as \codev{spect.plot\_amplitude = False} and \codev{spect.plot\_density = False} to compare the relative noise powers with respect to the connected baseline.
    }
...
\begin{figure}
    ...
    \caption[\imgsource{img/py/spectrometer/pyspeck_workflow.py}]{
        ...
        Hiding spectra that one is not interested in anymore is achieved through \codev{spect.hide(*range(2, 10))}.
        This is reversed by \codev{spect.show(*range(2, 10))}.
        Data can also be dropped (\codev{spect.drop(key)}) or deleted (\codev{spect.delete(key)}) from the internal cache and disk, respectively.
    }
...
This mode is facilitated by the \code{qutil.plotting.live_view} module~\cite{Hangleiter_qutil} that provides asynchronous plotting functionality based on \matplotlib.
The \code{live_view} module supports both the multithreading and multiprocessing paradigms for concurrency in order to keep the interpreter responsive.\sidenote[][*-1]{
