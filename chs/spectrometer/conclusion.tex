...
\AutoLettrine{In} \thispart, I introduced the \mintinline[breaklines,breakanywhere]{python}{python_spectrometer} software package for interactive and backend\hyp{}agnostic noise spectroscopy.
...
Another valuable addition would be supporting generic instruments abstracted by \qumada~\cite{Huckemann2026}.
...
\begin{marginlisting}
    ...
    \end{pycode}
    \caption{
        Template design for a proxy \codev{DAQ} implementation to stream noise spectra from an external measurement framework.
        The \codev{settings} attribute is a dictionary proxy shared between processes and used to pass acquisition parameters from the measurement framework to \pyspeck.
    }
...
\begin{listing}[htpb]
    ...
    \end{pycode}
    \caption{
        Proposed \codev{DAQ} workflow for estimating cross-spectra.
        Each hardware channel (same or different instruments) is assigned to a \codev{DAQ} object.
...
Further code adaptations would involve minor changes such as replacing the spectrum estimator with \code{scipy.signal.csd()}~\cite{CSDScipy} for \gls{csd} estimation\sidenote{
...
and making the plotting conform to complex data.\sidenote{
