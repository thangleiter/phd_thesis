...
\AutoLettrine{The} confocal microscope integrated into a millikelvin-temperature cryogen-free \gls{dr} for free-space optics experiments together with DC and AC electrical control was designed and set up by \citeauthor{Descamps2024}~\cite{Descamps2021,Descamps2024}.
...
If, on the other, the light emerges from a photonic crystal cavity for example, the far field pattern is close to a Gaussian mode and the considerations below need to be adjusted accordingly~\cite{Wu2024a}.
...
This yields for the emission angle of the marginal ray inside the semiconductor $\theta_{\mr{m}} = \arcsin(\flatfrac{\NA}{n}) \approx\qty{11}{\degree}$, implying that only a small fraction of light escapes the sample.
...
However, let us at least gain an intuition for the magnitude of these effects.
...
Moreover, the intensity $\propto\abs{\bvec{E}}^2$ does not depend strongly on the azimuthal angle $\phi$ at this distance, allowing us to approximate the field as rotationally invariant to estimate the coupling efficiency into the \gls{smf}.\sidenote[][*-3]{
...
The light that is collected and collimated by the objective lens next passes through the ocular lens in the detection arm which focuses it into the \gls{smf}.
...
Together with the collection efficiency (\cref{eq:setup:optics:coupling:efficiency:collection}) and accounting for the transmittance of the \gls{bs}, $\transmittance_{\mr{\acrshort{bs}}}\approx\qty{87}{\percent}$,\sidenote[][*-3]{
...
In \cref{sec:exp:tmm}, I analyze the reflectance of membrane samples in more detail using \gls{tmm} simulations and find that actually thin-film interference effects are expected to decrease the bare sample reflectance to around \qty{20}{\percent}.
...
\begin{figure}
    ...
    \caption[\imgsource{img/py/setup/imaging.py}]{
        ...
        Left column shows cropped images of the spot on an optical gate taken with the \acrshort{cmos} camera, while the middle column shows the fit to \cref{eq:setup:optics:coupling:intensity:tem00} rotated by a variable amount.
...
Allowing for ellipticity as well as a rotation of the coordinate system with respect to the axes of the \acrshort{cmos} camera, we can fit \cref{eq:setup:optics:coupling:intensity:tem00} to the pictures obtained previously.
...
As demonstrated by \textcite{Benelajla2021}, this technique enables extinction ratios\sidenote{
...
\end{equation}
where $P_{\mr{t}}$ is the transmitted power after, $P_{\mr{i}}$ the incident power before, and \transmittance the transmittance of the analyzer.
...
The dashed gray line indicates the axis along which the \gls{od} should theoretically be constant (rotating the \halfwave plate by $\Delta\phi$ changes the plane of polarization by $2\Delta\phi$, so the \quarterwave plate should need to be rotated by the same amount $2\Delta\phi$ to retain a constant angle between plane of polarization and fast axis of the waveplate).
...
\end{equation}
with $\bvec{\Delta\tilde{\phi}} = R(\pi/8+\theta)\bvec{\Delta\phi}$, $\bvec{\Delta\phi} = (\Delta\phi_{\quarterwave},\Delta\phi_{\halfwave})\transpose$ and $R(\theta)$ a rotation matrix.
Inserting into \cref{eq:setup:optics:od:eval} yields
\begin{equation}\label{eq:setup:optics:od:fit}
    \OD = \log_{10}\left(\frac{P \reflectance\lambda\eta_{\mr{o,tot}}\eta_{\QE}}{\transmittance h c}\right) - \log_{10}(\bvec{a}\transpose\bvec{\Delta\tilde{\phi}}^2 + \dot{\Phi}_0),
\end{equation}
a fit to which is shown in the right panel of \cref{fig:setup:optics:rejection}, displaying good agreement with the data.
From the fit, we extract $\theta=\qty{5.6}{\degree}$, indicating a non-negligible polarization dependence, as well as quadratic dispersion coefficients $\partial^2\OD/\bvec{\Delta\tilde{\phi}}^2=\qty[per-mode=symbol]{-0.048}{\per\txtdegree\squared}$ and \qty[per-mode=symbol]{-0.007}{\per\txtdegree\squared} perpendicular (parallel) to the axis along $\flatfrac{\pi}{8}+\theta$, respectively.
...
The \gls{hbt} interferometer consists of a symmetric ($\transmittance\div\reflectance=\num{50}\div\num{50}$) \acrlong{bs} with \glspl{spcm} on each exit port.
