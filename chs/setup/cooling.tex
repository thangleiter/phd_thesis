...
As further the radiative power scales with $T^4$ and there is at the same time more cooling power available at higher-temperature stages, installing windows there should result in an overall better performance.
...
Designed for two-qubit experiments with two-electron spin qubits by \textcite{Cerfontaine2019}, I tuned the device to host a single large \gls{qd} in the center of the device.
...
I applied a bias voltage at the Ohmic contact situated to the right of gate SA\sidenote{
    I use the same naming scheme for gate electrodes as \textcite[Chapter~7]{Cerfontaine2019}.
}
...
All but the four long gates (RFA--RFD) coming in from the top of the device were connected to the filtered DC lines as described in \citer{Descamps2021}.\sidenote[][*-3]{
...
From the fit we can extract the parameters $T = \qty{74.9}{\milli\kelvin} = \qty{6.5}{\micro\eV}/\kb$ and $\Gamma = \qty{0.524+-0.005}{\micro\eV} = \qty{6.08+-0.06}{\milli\kelvin}\times\kb$, confirming the sequential tunneling regime of $\Gamma\ll\kb T$.\sidenote[][*-5]{
