% mainfile: ../../main.tex
\chapter{Conclusion and outlook}\label{ch:setup:conclusion}
...
As shown by \textcite{Benelajla2021}, reflections of polarized non-plane-wave beams induce a modal transform into modes with nodes at the center that are spatially filtered by the \gls{smf} in a confocal geometry.
...
This situation is equivalent to that described by \textcite{Benelajla2021} if the emitted light is cross-polarized with respect to A and has a Gaussian mode profile.
...
As shown by \textcite{Steindl2023}, the giant cross-polarization extinction mechanism in a confocal geometry discovered by \textcite{Benelajla2021} is reduced in efficacy when additional reflections are introduced into the optical path.\sidenote{
    ...
    On the other hand, \textcite{Kuhlmann2013} found the \quarterwave plate to be crucial for compensating ellipticities introduced by the setup.
}
...
Low\hyp{}temperature effects such as superconductivity and other correlated electronic phases could be studied optically~\cite{Hadfield2016,Arora2020,Zhang2023}.
...
Examples of this include stacks of graphene or \gls{blg} with \glspl{tmd} such as \ch{MoS2} or \ch{WSe2}~\cite{Popert2022a,Masseroni2024,Xie2025,Seiler2025}.
The latter display giant exciton binding energies that are sensitive to the dielectric screening by their close environment, thus allowing to serve as optical sensors of electronic properties~\cite{Popert2022a,Tebbe2023}.
Recent experiments in this direction undertaken in the setup by \textcite{Tebbe2025} on \acrshort{blg}/\ch{WSe2} show promising first results on spin-orbit interaction proximity effects and magnetic phases in \gls{blg}~\cite{Icking2024}.
