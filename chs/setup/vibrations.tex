...
Assuming a perfect Gaussian (\gls{temmode}$_{00}$ mode) beam characterized by its waist radius $w_0$ at which the intensity drops to $1/e^2$ of its maximum value, the laser intensity profile in 1D is given by\todo{is the result the same for 2D? if yes, argue why 1D is sufficient!}
...
Just as the sensitivity $s$ is improved by increasing $\bar{\Phi}$ and $1-r$ and by decreasing $w_0$, so is the shot noise floor, albeit \emph{quadratically} in $w_0$ and $1-r$.
...
Based on the data from the accelerometer (circles), the microscope does not meet the targeted level of vibration isolation (VC-B, solid gray line) over three octaves.\sidenote{
    The sample effectively sits at the end of a large pendulum, so it does not come as a huge surprise the absolute level of vibrations is large.
}
\todo{finish this}
