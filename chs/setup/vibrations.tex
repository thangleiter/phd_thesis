...
Setting \code{procfn} to the linear transformation given in \cref{eq:setup:knife_edge:procfn} and measuring the counts registered by the \glspl{apd} using the Time Tagger,\footnote{
...
\end{equation}
or, for the flux noise $\delta\Phi=\flatfrac{\delta N(\dt)}{\dt}$,\sidenote{
    Note that we could have also arrived at this result directly by explicitly computing the autocorrelation function $\expval{\delta\Phi(t)\delta\Phi(t-\tau)}$ from \cref{eq:setup:flux_comb}.
}
