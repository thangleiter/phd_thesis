...
The latter are required in case the function modifies the frequencies.\sidenote[][*13]{
    One example is the \code{octave_band_rms()} function from the \code{qutil.signal_processing} subpackage~\cite{OctaveBandRmsQutil} that decimates frequencies to fractions of octave bands. See \cref{part:setup} for a use-case.
    % TODO: ref
}
