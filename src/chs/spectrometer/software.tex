...
\begin{marginlisting}
    ...
    \end{py}
    \caption{
        Resolved settings for the same input parameters as in \cref{lst:speck:daq_settings} but for the \code{ZurichInstrumentsMFLIScope} backend with hardware constraints on \code{n_pts} and \texttt{fs}. % for some reason src.tex errors for \code{fs} or \codev{fs}
    }
