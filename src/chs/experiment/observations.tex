...
In the first case, the previously assumed translational invariance along $x$ leading to a plane-wave spatial dependence is replaced by a superposition of plane waves weighted with a normally distributed amplitude,\sidenote{
    \Ie, the inverse Fourier transform of $\mc{E}_0(k_x)$.
}
\begin{equation}\label{eq:exp:tmm:gauss:x}
    E_{y,i}(x) = \exp(\i k_x x)\rightarrow \int\ddf{k_x}\mc{E}_0(k_x) \exp(\i k_x x),
\end{equation}
...
\end{equation}
so that
\begin{equation}\label{eq:exp:tmm:gauss:xz}
    E_{y,i}(x, z) = E_{y,i}(z) E_{y,i}(x)
\end{equation}
with
\begin{equation}\label{eq:exp:tmm:gauss:z}
    E_{y,i}(z) = A_{i}^{-}\exp\lbrace\i\gamma_i[z-z_{i+1}]\rbrace + B_{i}^{+}\exp\lbrace -\i\gamma_i[z-z_{i}]\rbrace
\end{equation}
