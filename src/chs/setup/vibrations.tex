...
Below $\omega=\sqrt{2}\omega_0$ (vertical dotted line), the suspension in fact leads to amplification of external impulses.
...
I illuminated the sample with the white light, positioned the spot on the edge of the optical gate, and imaged the sample with the \cmoscam.
...
\end{equation}
Again performing a linear fit (shown in the upper panel of \cref{fig:setup:vibrations:calibration:pos_vs_cps}) to the data for different voltages then results in the linear transformation from DC voltage to position.
...
\begin{figure}
    ...
    \caption[\imgsource{img/py/setup/vibration_spectroscopy.py}]{
        ...
        The total $\rms_S \approx \qty{100}{\nano\meter}$ in working mode is below the typical \unit{\micro\meter} feature size.
    }
...
In that sense the optical method gives us the more pertinent results because only the displacements \enquote{experienced} by the light travelling through the microscope ultimately matter.
...
\end{equation}
To convert to the displacement noise \gls{psd}, we can simply convert units using the calibration derived above because if $N\sim\poisson(\bar{N})$ then so $y\sim\poisson(\flatfrac{\bar{N}\fs}{a})$ where $a$ is the slope of the calibration converting displacements to count rates (\cref{fig:setup:vibrations:calibration:pos_vs_cps}).
Hence,\sidenote{
...
\begin{equation}\label{eq:setup:poisson:psd:displ}
   S_{\delta y(\dt)}(\omega) = \frac{\bar{N}}{\fs}\times\left(\frac{\fs}{a}\right)^2 = \frac{\bar{\Phi}}{a^2}
\end{equation}
