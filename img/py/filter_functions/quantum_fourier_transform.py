...
if RUN_SIMULATION:
    ...
    four_qubit_pulses['Y_pi2'][(0,)] = ff.extend([(pulses['Y_pi2'], 0, identifier_mapping)],
                                                 ...
                                                 cache_filter_function=True,
                                                 show_progressbar=SHOW_PROGRESSBAR)
    four_qubit_pulses['CZ_pi8'][(0, 1)] = ff.extend(
        [(pulses['CZ_pi8'], (0, 1), identifier_mapping)],
        N=n_qubits, omega=omega,
    ...
    assert ff.util.oper_equiv(qt_qft(4), qft_pulse_correl.total_propagator)
    assert ff.util.oper_equiv(qt_qft(4), qft_pulse_echo_correl.total_propagator)

    # %%% Infidelities
    S0 = 2.2265e-6

    infids = {}
    infids['$1/f$ without echo'] = ff.infidelity(
        qft_pulse_correl, S, omega, which='correlations',
        n_oper_identifiers=['$\\sigma_y^{(3)}$']
    )
    infids['$1/f$ with echo'] = ff.infidelity(
        qft_pulse_echo_correl, S, omega, which='correlations',
        n_oper_identifiers=['$\\sigma_y^{(3)}$']
    )
    infids['White without echo'] = ff.infidelity(
        qft_pulse_correl, np.full_like(omega, S0), omega, which='correlations',
        n_oper_identifiers=['$\\sigma_y^{(3)}$']
    )
    infids['White with echo'] = ff.infidelity(
        qft_pulse_echo_correl, np.full_like(omega, S0), omega, which='correlations',
        n_oper_identifiers=['$\\sigma_y^{(3)}$']
    )
    ...
    # %%% Save pulses, infidelities
    for name, pulse in pulses.items():
        save_pulse_sequence(DATA_PATH / f"pulse_sequence_{name}.npz",
                            pulse, data=False, frequency_data=True, intermediates=False)
    for pulse in ('qft_pulse', 'qft_pulse_echo'):
        save_pulse_sequence(DATA_PATH / f"pulse_sequence_{pulse}.npz",
                            prune_nopers(locals()[pulse]),
                            data=False, frequency_data=True, intermediates=False)
    np.savez(DATA_PATH / 'qft_infidelities.npz', **infids, S0=S0)
else:
    # %%% Load pulses
    pulses = {path.stem[len('pulse_sequence_'):]: load_pulse_sequence(path)
              for path in DATA_PATH.glob('pulse_sequence_*.npz') if 'qft_pulse' not in path.stem}
    qft_pulse = load_pulse_sequence(DATA_PATH / 'pulse_sequence_qft_pulse.npz')
    qft_pulse_echo = load_pulse_sequence(DATA_PATH / 'pulse_sequence_qft_pulse_echo.npz')
    with np.load(DATA_PATH / 'qft_infidelities.npz') as arch:
        infids = dict(arch)
        S0 = infids.pop('S0').item()
...
for i, (name, pulse) in enumerate(pulses.items()):
    ...
    *_, leg = ff.plotting.plot_filter_function(
        pulse, fig=fig, axes=axes[i, 1], cycler=cycle,
        omega=ff.util.get_sample_frequencies(pulse, n_samples=500),
        omega_in_units_of_tau=False, n_oper_identifiers=n_identifiers[name]
    )
    leg.remove()
